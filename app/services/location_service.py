...
def get_location_by_id(db: Session, location_id: int, parent_id: int) -> Location:
    """
    ...
    Raises:
        HTTPException 404: Si la location n'existe pas ou l'enfant n'appartient pas au parent
    """
    # Récupérer les IDs des enfants du parent
    child_ids = db.query(Child.id).filter(Child.parent_id == parent_id).all()
    child_ids = [child_id[0] for child_id in child_ids]
